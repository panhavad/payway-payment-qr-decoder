...
function getSelectedCrcWidth() {
    ...
    else if (document.getElementById('radiobuttoncrc64').checked) {
        selCrcWidth = 64;
    }
    else {
...
function getInputData(inputText) {
    ...
    else if (document.getElementById('radiobuttoninputdatabinary').checked) {
        return stringUtil.getCharacterByteArrayFromBinaryString(inputText);
    }
    else {
        /* invalid state */
        return undefined;
...
function calcButton_clicked() {
    ...
    var selCrcWidth = 16; //for CRC16
    var crcParams = getDataBaseEntryFromEntry(selCrcWidth, 4); //4 is CRC16_CCITT_FALSE
...
function printResultCrc(selCrcWidth, crcValue) {
    if (selCrcWidth == 64) {
        document.getElementById('resultLabel').innerHTML =
            '<span style="color:green">' + crcValue.toHexString() + '</span> ';
    }
...
function printLookupTable(selCrcWidth, crc) {
    ...
    if (selCrcWidth == 64) {
        for (var i = 0; i < 256; i++) {
            tableStr += lookupTable[i].toHexString();
            tableStr += ' ';
            if (i % 8 == 7) {
                tableStr += '<br/>';
            }
...
function predefinedSelectChangeEvent() {
    var selCrcWidth;
    selCrcWidth = getSelectedCrcWidth();
    ...
    if (crcParams.width == 64) {
        document.getElementById('textBoxPolynomial').value = crcParams.polynomial.toHexString();
        document.getElementById('textBoxInitialValue').value = crcParams.initial.toHexString();
        document.getElementById('textBoxFinalXorValue').value = crcParams.finalXor.toHexString();
    }
    else {
...
function pageOnLoad() {
    ...
    document.getElementById('selectpredefined').selectedIndex = 0;
    predefinedSelectChangeEvent();
    crcWidthInputType_changed("CRC16");
